...
def plot_tree(tree, keep_out_boxes):
    '''
    ...
    for box in keep_out_boxes:
        rect = patches.Rectangle((box.x0, box.y0), box.x1-box.x0, box.y1-box.y0, linewidth=1, edgecolor='r', facecolor='none')
        ax.add_patch(rect)
    # Add the tree to the plot
    for node in tree:
        if node.get_parent() is not None:
            parent_x, parent_y = node.get_parent().get_position()
            line = lines.Line2D([parent_x, node._x], [parent_y, node._y], color='b')
            ax.add_line(line)
    # Add the start and goal nodes to the plot
    start_node = tree[0]
    goal_node = tree[-1]
    ax.plot(start_node._x, start_node._y, 'go', markersize=10, label='Start')
    ax.plot(goal_node._x, goal_node._y, 'ro', markersize=10, label='Goal')
    ax.legend()
    plt.show()
...
def main():
    ...
    tree = rrt(x_min,x_max,y_min,y_max,start_coords,gate_coords,keep_out_boxes)

    # plot the tree
    plot_tree(tree, keep_out_boxes)
